...
inline fm_float wTx(
    ...
    if (do_update) {
        // weight w0
        fm_float sign = model.w0_z > 0 ? 1.0 : -1.0;
        if (sign * model.w0_z <= L1) {
            model.w0 = 0;
        } else {
            model.w0 = -(model.w0_z - sign * L1) / (L2 + (beta + sqrt(model.w0_n)) / alpha);
        }
        fm_float w0_grad = kappa;
        fm_float sigma = (sqrt(model.w0_n + w0_grad * w0_grad) - sqrt(model.w0_n)) / alpha;
        model.w0_z += (w0_grad - sigma * model.w0);
        model.w0_n += w0_grad * w0_grad;
        for (fm_node *node1 = begin; node1 != end; node1++) { // one feature in a sample
            ...
            sign = unit1.w_z > 0 ? 1.0 : -1.0;
            if (sign * unit1.w_z <= L1) {
                unit1.w = 0;
            } else {
                unit1.w = -(unit1.w_z - sign * L1) / (L2 + (beta + sqrt(unit1.w_n)) / alpha);
            }
            fm_float w_grad = kappa * value1;
            sigma = (sqrt(unit1.w_n + w_grad * w_grad) - sqrt(unit1.w_n)) / alpha;
            unit1.w_z += (w_grad - sigma * unit1.w);
            unit1.w_n += w_grad * w_grad;
            // latent vector i
            for (fm_int f = 0; f < model.k; f++) {
                sign = unit1.v_z.at(f) > 0 ? 1.0 : -1.0;
                if (sign * unit1.v_z.at(f) <= L1) {
                    unit1.v.at(f) = 0;
                } else {
                    unit1.v.at(f) = -(unit1.v_z.at(f)- sign * L1) / (L2 + (beta + sqrt(unit1.v_n.at(f))) / alpha);
                }
                fm_float v_grad = 0;
                ...
                v_grad = kappa * v_grad;
                sigma = (sqrt(unit1.v_n.at(f) + v_grad * v_grad) - sqrt(unit1.v_n.at(f))) / alpha;
                unit1.v_z.at(f) += (v_grad - sigma * unit1.v.at(f));
                unit1.v_n.at(f) += v_grad * v_grad;
            }  
        }
    } else {
        // weight w0
        ...
        for (fm_node *node = begin; node != end; node++) {
            ...
            res += unit.w * value;
            //cout << "res2:" << unit.w << " " << value << " " << res << endl;
        }
        // latent vector i
        ...
        for (fm_int f = 0; f < model.k; f++) {
            ...
            for (fm_node *node = begin; node != end; node++) {
                ...
                //cout << idx << " " << unit.v.at(f) << " " << value << " " << sum_square << " " << square_sum << endl;
            }
            latent_res += (sum_square * sum_square - square_sum);
            //cout << "res3:" << sum_square << " " << square_sum << latent_res << endl;
        }
        res += (latent_res * 0.5);
    }
    return res;
...
fm_model init_model(fm_int n, fm_parameter param) {
    ...
    for (fm_int i = 0; i < model.n; i++) {
        ...
        for (fm_int j = 0; j < model.k; j++) {
            ...
            unit.v_n.push_back(0);
            unit.v_z.push_back(0);
        }
...
#pragma omp parallel for schedule(static) reduction(+: loss)
            ...
            for (fm_int ii = 0; ii < l; ii++) {
                ...
                if(do_update) {
                    ...
                    //cout << "t:" << t << " expnyt:" << expnyt << " " << kappa << endl;
                    wTx(begin, end, model, kappa, param.alpha, param.beta, param.L1, param.L2, true);
                }
            }
        }
...
fm_int fm_save_model(fm_model &model, string path) {
    ...
    f_out << "n " << model.n << "\n";
    f_out << "k " << model.k << "\n";
    f_out << "bias " << model.w0 << " " << model.w0_n << " " << model.w0_z << "\n";

    for (fm_int i = 0; i < model.n; i++) {
        fm_weight_unit &unit = model.weight_map.find(i)->second;
        f_out << "w " << i << " " << unit.w << " " << unit.w_n << " " << unit.w_z << "\n";
    }
    for (fm_int i = 0; i < model.n; i++) {
        f_out << "v " << i;
        fm_weight_unit &unit = model.weight_map.find(i)->second;
...
fm_model fm_load_model(string path) {
    ...
    f_in >> dummy >> model.n
         >> dummy >> model.k
         >> dummy >> model.w0 >> model.w0_n >> model.w0_z;
    ...
    for (fm_int i = 0; i < model.n; i++) {
        ...
        for (fm_int j = 0; j < model.k; j++) {
            fm_float val = 0;
            f_in >> val;
            unit.v_n.push_back(val);
        }
        for (fm_int j = 0; j < model.k; j++) {
            fm_float val = 0;
            f_in >> val;
            unit.v_z.push_back(val);
        }
    }
    return model;
}
